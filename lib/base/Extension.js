...
let proxyDownloader = null

async function getProxyDownloader() {
  if (proxyDownloader) return proxyDownloader
  try {
    proxyDownloader = await import('../proxyDownloader.js')
  } catch {}
...
export default class Extension {
  ...
  /**
   * Detect if URL is an album/collection page based on config patterns
   * @param {string} url - URL to check
   * @returns {boolean} True if URL matches album patterns
   */
  isAlbumUrl(url) {
    if (!url || !this.config.domains_includes_album) return false

    for (const pattern of this.config.domains_includes_album) {
  ...
  async cloudflareFetch(url) {
    const proxy = await getProxyDownloader()
    if (!proxy?.cloudflareFetch) throw new Error('Cloudflare fallback not available')
    return proxy.cloudflareFetch(url)
  }
